...
def MPU_Init():
    # Escribe en el registro de tasa de muestreo
    bus.write_byte_data(Device_Address, SMPLRT_DIV, 7)
    
    # Escribe en el registro de administración de energía
    bus.write_byte_data(Device_Address, PWR_MGMT_1, 1)
    
    # Escribe en el registro de configuración
    bus.write_byte_data(Device_Address, CONFIG, 0)
    
    # Escribe en el registro de configuración del giroscopio
    bus.write_byte_data(Device_Address, GYRO_CONFIG, 24)
    
    # Escribe en el registro de habilitación de interrupción
...
def read_serial():
    with serial.Serial('/dev/ttyUSB1', 115200) as ser:  # Reemplaza '/dev/ttyUSB1' por tu puerto serial
        while True:
